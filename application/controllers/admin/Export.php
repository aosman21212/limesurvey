...
if (Yii::app()->request->getPost('response_id')) {
    ...
    $startChatStartDatetime = Yii::app()->request->getPost('start_chat_start_datetime'); // Retrieve the "Start Chat Start Datetime" value
    $endChatStartDatetime = Yii::app()->request->getPost('end_chat_start_datetime'); // Retrieve the "End Chat Start Datetime" value

    $whatsappNumber = Yii::app()->request->getPost('whatsapp_number'); // Retrieve the "WhatsApp Number" value

    $servedBy = Yii::app()->request->getPost('served_by'); // Retrieve the "Served By" value
    $submittedStatus = Yii::app()->request->getPost('survey_submitted_status'); // Retrieve the "Submitted Status" value

    $interval = Yii::app()->request->getPost('interval'); // Retrieve the selected interval
    if ($interval) {
        switch ($interval) {
            case 'hourly':
    ...
    // Add date range filter based on submitdate field
    if ($startDate && $endDate) {
        $sSubmitDateFilter = "{{survey_{$iSurveyID}}}.submitdate BETWEEN '{$startDate}' AND '{$endDate}'";
    } else {
        $sSubmitDateFilter = '';
    }
    // Add chat start datetime range filter
    if ($startChatStartDatetime && $endChatStartDatetime) {
        // Assuming 'chat_start_datetime' is the field name in the database
        $sChatStartDatetimeFilter = "{{survey_{$iSurveyID}}}.chat_start_datetime BETWEEN '{$startChatStartDatetime}' AND '{$endChatStartDatetime}'";
    } else {
        $sChatStartDatetimeFilter = '';
    }
    ...
    // Add survey sent date range filter
    ...
    // Add "Served By" filter
    if ($servedBy) {
        // Assuming 'served_by' is the field name in the database
        $sServedByFilter = "{{survey_{$iSurveyID}}}.served_by = '{$servedBy}'";
    } else {
        $sServedByFilter = '';
    }

    // Add "WhatsApp Number" filter
    if ($whatsappNumber) {
        // Assuming 'whatsapp_number' is the field name in the database
        $sWhatsAppNumberFilter = "{{survey_{$iSurveyID}}}.whatsapp_number = '{$whatsappNumber}'";
    } else {
        $sWhatsAppNumberFilter = '';
    }

    // Add "Submitted Status" filter
    if ($submittedStatus) {
        // Assuming 'survey_submitted_status' is the field name in the database
 ...
 if ($startSurveySubmittedDate && $endSurveySubmittedDate) {
     $sSurveySubmittedDateFilter = "{{survey_{$iSurveyID}}}.survey_submitted_date BETWEEN '{$startSurveySubmittedDate}' AND '{$endSurveySubmittedDate}'";
 } else {
     $sSurveySubmittedDateFilter = '';
 }

 $startSurveySubmittedDate = Yii::app()->request->getPost('start_survey_submitted_date');
 $endSurveySubmittedDate = Yii::app()->request->getPost('end_survey_submitted_date');
...
// if (empty($responses)) {
//     // No results found, redirect back to the previous page
//     $url = Yii::app()->request->getUrlReferrer();
//     Yii::app()->request->redirect($url);
// } else {
//     // Results found, proceed with exporting
//     $resultsService->exportResponses($iSurveyID, $explang, $sExportType, $options, $sFilter);
// }
